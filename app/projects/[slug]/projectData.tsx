...
const severityStyles: Record<NonNullable<Project["findings"]>[number]["severity"], string> = {
        ...
        <section className="mt-8">
          ...
          <div className="mt-3 space-y-4">
            {project.findings.map((f, i) => (
              <div key={`${f.title}-${i}`} className="rounded-xl border border-neutral-200 p-4">
      ...
      {project.additionalSections?.length ? (
        <section className="mt-8 space-y-6">
          {project.additionalSections.map((sec, i) => (
            <div key={`${sec.title}-${i}`} className="rounded-xl border border-neutral-200 p-4">
              <div className="mb-2 flex items-center gap-2">
                {sec.icon ? <span className="text-neutral-700">{sec.icon}</span> : null}
              ...
              <ul className="mt-3 list-disc pl-6">
                {project.blogs.map((b, i) => (
                  <li key={`${b.title}-${i}`}>
                    ...
                    {b.description ? (
                      <span className="ml-2 text-sm text-neutral-600">— {b.description}</span>
                    ) : null}
                  </li>
                ))}
              </ul>
...
// (Optional) Generate static params if you statically pre-render project pages.
